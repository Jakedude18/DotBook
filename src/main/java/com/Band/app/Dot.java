...
class Dot {
    ...
    Dot(int set, int counts, String coordinate) {
        if (Pattern.matches("^(s[1,2] [1-4] ((in|out)|on) ([1-4]?5|([1-5]0))|on 50) ((([1-9]|10) (fbh|bfh))|(([1-9]|1[0-6]) (((bf|fb)s)|((b{2}|f{2})h))))$", coordinate)) {
            ...
            m.find();
            this.coordinate = new Pair<>(coordinate.substring(0, m.end() - 1), coordinate.substring(m.end()));
            if (onCoordinate) {
                coordinate = String.format("%s 0 in %s ", coordinate.substring(0, 1).toUpperCase() + coordinate.substring(1,2), getCoordinateX().substring(getCoordinateX().length() - 2));
